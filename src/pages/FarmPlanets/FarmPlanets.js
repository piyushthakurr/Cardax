...
import Searchicon from "../../assets/images/token_icons/saitamaIcons/searchIcon.svg";
...
const FarmPlanets = (props) => {
  ...
  const handleTab = (data) => {
    history.push(`${rootName}/farms/${data}`);
  };
  ...
  const depositWithdraw = async (type) => {
    ...
    if (type === "deposit") {
      ...
      value = (value * 10 ** stakeData.lpTokenDetails.decimals).toFixed();
      // console.log(value);
      const amount = value.toLocaleString("fullwide", { useGrouping: !1 });
  ...
  return (
    <div className="container_wrap farmpln frm_plnet_disply">
      <div className="farm_srchbar">
        <Container>
          <h4 className="title_hd text-center">Farming</h4>
          <div className="contain_area">
            <div className="desc">
              <p>
                There is so much to explore! SaitaSwap offers multiple farming
                oppurtunities to you. Get amazing rewards by staking your LP
                tokens in return for additional SAITAMA Tokens.
              </p>
            </div>
