...
// import { Apiget } from "../Trade/DollarPrice.js";
...
const AddLiquidity = (props) => {
  ...
  const onHandleSelectCurrency = async (token, selecting) => {
    ...
    if (a1 && a2) {
      ...
      if (currentPairAddress !== "0x0000000000000000000000000000000000000000") {
        ...
        console.log("thi is rereerves", reserves);
    ...
    if (tokenType === "TK1") {
      ...
      if (r && tokenOne.address && tokenTwo.address && amount > 0) {
        ...
        if (currentPairAddress) {
          // alert("in currentPair");
          console.log("this os currentPair", currentPairAddress);
          ...
          console.log("these are reserves", reserves);
  ...
  const checkAddLiquidity = async () => {
    if (!isUserConnected) {
      ...
      //   tokenOneValue
      // );
      if (Number(tokenOneBalance) < Number(tokenOneValue)) {
        return toast.error(
          `Wallet have insufficient ${tokenOne.symbol} balance!`
        );
      }
      if (Number(tokenTwoBalance) < Number(tokenTwoValue)) {
        return toast.error(
  ...
  // console.log("78788798", tokenTwoBalance);
  return (
    <>
      <Container fluid className="swapScreen comnSection">
        <CardCustom>
          ...
          <Col className="swapBtn_col mt-4">
            {handleApprovalButton("TK1")}
            {handleApprovalButton("TK2")}
